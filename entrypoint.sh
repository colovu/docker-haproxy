...
docker_create_user_directories() {
	...
	# 检测指定文件是否存在，如果不存在则拷贝
	[ ! -e /srv/conf/${APP_NAME}/${APP_NAME}.cfg ] && cp /etc/${APP_NAME}/${APP_NAME}.cfg /srv/conf/${APP_NAME}/${APP_NAME}.cfg
	...
	if [ "$user_id" = '0' ]; then
		...
		chmod 755 /etc/${APP_NAME} /var/log/${APP_NAME} /var/cache/${APP_NAME} /var/run/${APP_NAME} /srv/conf/${APP_NAME} || :
